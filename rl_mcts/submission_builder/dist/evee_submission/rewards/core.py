...
def take_prize_shape(obs: Observation, your_index: int, nn_value: float, weight: float = 0.1) -> float:
    """Reward taking your own prize cards this step (KO'd opponent, claim a prize).

    Counts this step's MOVE_CARD log events moving a card from your PRIZE to your
    HAND. Each prize taken adds weight (2 on an ex/mega-ex KO, since two/three move
    in the same step).
    """
    taken = sum(
        1
...
def attached_energy_shape_attack(obs: Observation, your_index: int, nn_value: float, weight: float = 0.1) -> float:
...
def opp_active_is_ex(obs: Observation, your_index: int) -> bool:
    ...
    if not active or active[0] is None:
        return False
    return is_ex(active[0])


def is_ex(card) -> bool:
    """True if card id is a Pokemon ex (includes Mega ex). Accepts object or raw card id."""
    cd = CARD_BY_ID.get(card_id(card))
    return cd is not None and cd.ex


def is_evolution(card) -> bool:
    """True if card id is an evolution (Stage 1 or Stage 2). Accepts object or raw card id."""
    cd = CARD_BY_ID.get(card_id(card))
    return cd is not None and (cd.stage1 or cd.stage2)


def opp_field_card_ids(obs: Observation, your_index: int) -> list[int]:
    """Card ids of opponent's in-play pokemon (active + bench), skipping empty slots."""
    opp = obs.current.players[1 - your_index]
    return [p.id for p in board(opp) if p is not None]
