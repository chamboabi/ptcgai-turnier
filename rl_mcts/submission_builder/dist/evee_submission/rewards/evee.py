...
# Opponent's active ex id -> our counter card id to race into play against it.
# Picked fresh each search root from obs0, since the matchup can change turn to
# turn. Fill in real matchups; unlisted opponent ids fall back to 0 (no-op, see
# race_card_shape).

# TODO: implement this later right now rule based mechanism
COUNTER_BY_OPP_EX_ID: dict[str, int] = {
    # opp_ex_id: counter_id
}
...
def should_race_sylveon(obs: Observation, your_index: int) -> int:
    """Returns the sylveon card to race if any of the opponent cards is a ex card."""
    opp_active_cards = opp_field_card_ids(obs, your_index)
    for opac in opp_active_cards:
        if is_ex(opac):
            return 330
    return 0


def opp_active_id(obs: Observation, your_index: int) -> int | None:
...
def make_evee_shape(obs0: Observation, your_index: int):
    ...
    """
    card_to_race = should_race_sylveon(obs0, your_index)
    race_eevee = functools.partial(race_card_shape, poke_id=card_to_race)
    weights = {**WEIGHTS, race_eevee: RACE_WEIGHT}
    needs_baseline = _NEEDS_BASELINE | {race_eevee}

    base = make_base_shape(obs0, your_index)
    evee = make_compound_shape(weights, needs_baseline, obs0, your_index)
