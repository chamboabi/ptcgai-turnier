...
Bundled by submission_builder/build.py from a data/player/<name>/ package into
a folder holding:
    main.py           (this file, generated with that player's config baked in)
    deck.csv          (60 card IDs, one per line)
    player_reward.py  (the player's reward.py, make_shape(obs0, your_index))
...
# Reward shape from the bundled player package (per-turn baseline built via
# shape_factory in mcts_agent).
_REWARD = RewardFn(terminal=win_loss_terminal, shape=identity_shape, shape_factory=player_reward.make_shape)
