"""Kaggle entry point: Mega Starmie ex deck piloted by determinized rollout search.

On the initial observation, returns the deck. Otherwise: at a MAIN decision it runs a
time-bounded determinized search (our starmie_pilot as the rollout policy for us, the
generic KO-aware heuristic for the guessed opponent); everywhere else, and whenever
search has no time/signal, it falls back to the starmie_pilot. Never raises; always
...
from starmie_pilot import make_agent as make_starmie  # noqa: E402
...
def _make_our_pilot(deck):
    al, cl = _lookups()
    return make_starmie(deck, al, cl)
